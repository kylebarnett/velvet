...
type FounderDashboardBuilderProps = {
  companyId: string;
  companyName: string;
...
export function FounderDashboardBuilder({
  companyId,
  companyName,
  companyIndustry,
  availableMetrics,
  ...
  const [widgets, setWidgets] = React.useState<Widget[]>(() => {
    ...
    if (selectedView) return parseLayout(selectedView.layout);
    const industryTemplate = templates.find(
      (t) => t.target_industry === companyIndustry && t.is_system,
    );
    if (industryTemplate) return parseLayout(industryTemplate.layout);
    ...
    try {
      if (currentView) {
        const res = await fetch(
          `/api/founder/dashboard-views/${currentView.id}`,
          {
            method: "PUT",
        ...
        const res = await fetch("/api/founder/dashboard-views", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            companyId,
